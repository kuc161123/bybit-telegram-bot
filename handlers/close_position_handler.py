#!/usr/bin/env python3
"""
Close Position Handler - Handles individual position closing with confirmation
...
async def handle_list_positions(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if main_positions:
            text += f"<b>💼 MAIN ACCOUNT ({len(main_positions)} positions)</b>\n"
            for pos in main_positions:
                ...
                text += f"   Size: {size}\n"
                text += f"   PnL: ${format_mobile_currency(Decimal(str(pnl)))}\n"
        ...
        if mirror_positions:
            text += f"\n<b>🔄 MIRROR ACCOUNT ({len(mirror_positions)} positions)</b>\n"
