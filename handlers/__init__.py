...
from .conversation import (
    ...
    # NEW: Conservative margin handlers
    handle_margin_conservative_callback,
    ...
    try:
        ...
        conv_handler = ConversationHandler(
            ...
            states={
                ...
                # NEW: Conservative margin selection for GGShot
                MARGIN_CONSERVATIVE: [
...
def setup_callback_handlers(app):
    ...
    try:
        ...
        app.add_handler(CallbackQueryHandler(handle_dashboard_callbacks, pattern="^dashboard$"))
...
def setup_enhanced_stats_handlers(app):
    """Setup enhanced statistics handlers for conservative approach"""
    try:
...
def setup_debug_handler(app):
    ...
    async def debug_all_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        query = update.callback_query
        if query:
            # Ignore conversation callbacks and dashboard callbacks - they're handled by specific handlers
            conversation_patterns = [
                'conv_side:', 'conv_approach:', 'conv_leverage:', 'conv_margin:',
                'conv_margin_pct:', 'conv_margin_cons:',
                ...
                'start_conversation',  # Trade conversation starter
                'close_position:', 'confirm_close_position:', 'cancel_close_position',
                'list_positions',  # Position close handlers
                'refresh_dashboard', 'dashboard', 'back_to_dashboard'  # Dashboard handlers (working correctly)
            ]
    ...
    logger.info("🔍 Debug handler registered for unhandled callbacks")
...
def setup_alert_handlers(app):
    ...
    try:
        ...
        logger.info("🔍 Registering show_all_positions handler")
        ...
        logger.info("🔍 Registering show_pnl_details handler")
        ...
        logger.info("🔍 Registering show_monitors handler")
...
def setup_handlers(app: Application):
    """Setup all enhanced bot handlers with conservative approach support"""
    logger.info("Setting up enhanced conservative approach trading bot handlers...")
    ...
    try:
        ...
        logger.info("🚨 Emergency shutdown handler loaded!")
        logger.info("   • /emergency - Close all positions and cancel all orders")
        logger.info("   • Two-step confirmation required")
        logger.info("   • 5-minute cooldown between uses")
    except Exception as e:
        logger.error(f"❌ Error loading emergency handler: {e}")
    ...
    try:
        ...
        logger.info("✅ Manual Rebalancer commands loaded!")
        logger.info("   • /rebalance - Manual position rebalancing")
        logger.info("   • /rebalancer - Show rebalancer status")
        logger.info("   • ⚖️ SAFE: Only adjusts quantities, preserves trigger prices")
    except Exception as e:
    ...
    # AI handlers with conservative approach support
