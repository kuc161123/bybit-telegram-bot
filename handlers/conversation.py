...
UPDATED: Conservative approach only (Fast approach removed)
...
# Telegram message retry configuration - Enhanced parameters
MAX_TELEGRAM_RETRIES = 5  # Increased from 3 to handle more timeout scenarios
TELEGRAM_RETRY_DELAY = 3  # Increased from 2 seconds for better recovery
TELEGRAM_TIMEOUT = 30  # Timeout for individual requests

async def send_message_with_retry(bot, chat_id: int, text: str, parse_mode=None, reply_markup=None, max_retries: int = MAX_TELEGRAM_RETRIES):
    """Send a Telegram message with enhanced retry logic for timeout errors"""
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                backoff_time = TELEGRAM_RETRY_DELAY * (2 ** attempt) + (0.1 * attempt)
                logger.warning(f"Telegram timeout on attempt {attempt + 1}/{max_retries}, retrying in {backoff_time:.1f}s...")
                await asyncio.sleep(backoff_time)
            else:
                logger.error(f"Telegram timeout after {max_retries} attempts: {e}")
            ...
            if "429" in str(e):  # Rate limit error
                ...
                logger.error(f"Telegram error (non-retryable): {e}")
                return None
        except Exception as e:
            logger.error(f"Unexpected error sending message: {e}")
            return None

# Define conversation states - ENHANCED with GGShot screenshot strategy and conservative approach only
...
async def start_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    context.chat_data["in_conversation"] = True  # Flag to track conversation state
    ...
    try:
        ...
        try:
            ...
            pass
        context.chat_data["in_conversation"] = False  # Clear conversation flag
...
async def handle_approach_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    query = update.callback_query
    logger.info(f"🎯 handle_approach_callback called - data: {query.data if query else 'No query'}")
    try:
        await query.answer()
    except:
        pass
    ...
    if not query.data.startswith("conv_approach:"):
        # Invalid callback data, stay in approach selection
        logger.warning(f"Invalid approach callback data: {query.data}")
        return APPROACH_SELECTION
    ...
    if approach == "conservative":
        ...
        # Default to conservative for any unknown approach
        approach_emoji = "🛡️"
        approach_text = "Conservative Limits"
...
async def show_extracted_parameters_confirmation(context: ContextTypes.DEFAULT_TYPE, chat_id: int, extracted_data: dict) -> int:
    ...
    strategy_type = extracted_data.get("strategy_type", "conservative")
    ...
    if strategy_type == "conservative":
        ...
        confirmation_msg = (
            ...
            f"{accuracy_msg}\n"
            f"📊 <b>EXTRACTED VALUES:</b>\n\n"
            f"💰 <b>Entry Price:</b> {format_value_with_edit_indicator(params.get(PRIMARY_ENTRY_PRICE), 'entry', edited_fields)}\n"
            f"🎯 <b>Take Profit:</b> {format_value_with_edit_indicator(params.get(TP1_PRICE), 'tp1', edited_fields)} (100%)\n"
            f"🛡️ <b>Stop Loss:</b> {format_value_with_edit_indicator(params.get(SL_PRICE), 'sl', edited_fields)}\n\n"
            f"💡 <i>Review values and edit if needed</i>\n"
        )
    ...
    if strategy_type == "conservative":
        ...
        limits_btn_text = "📝 Edit Limit Orders" if not limits_edited else "✏️ Edit Limit Orders (Modified)"

        # Check if any TPs have been edited
        tps_edited = any(f"tp{i}" in edited_fields for i in range(1, 5))
        tps_btn_text = "🎯 Edit Take Profits" if not tps_edited else "✏️ Edit Take Profits (Modified)"

        # Check if SL has been edited
        sl_btn_text = "🛡️ Edit Stop Loss" if "sl" not in edited_fields else "✏️ Edit Stop Loss (Modified)"
        ...
        entry_btn_text = "💰 Edit Entry Price" if "entry" not in edited_fields else "✏️ Edit Entry Price (Modified)"
        tp_btn_text = "🎯 Edit Take Profit" if "tp1" not in edited_fields else "✏️ Edit Take Profit (Modified)"
        sl_btn_text = "🛡️ Edit Stop Loss" if "sl" not in edited_fields else "✏️ Edit Stop Loss (Modified)"

        keyboard_buttons.extend([
            [InlineKeyboardButton(entry_btn_text, callback_data="ggshot_edit_entry")],
            [InlineKeyboardButton(tp_btn_text, callback_data="ggshot_edit_tp_conservative")],
            [InlineKeyboardButton(sl_btn_text, callback_data="ggshot_edit_sl")]
        ])
...
async def handle_ggshot_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == "ggshot_edit_limits":
    ...
    elif query.data == "ggshot_edit_tp_conservative":
        # Show TP editing interface for conservative approach
        ...
        return await show_extracted_parameters_confirmation(context, query.message.chat.id,
            {"success": True, "parameters": context.chat_data, "strategy_type": context.chat_data.get(ORDER_STRATEGY, "conservative")})
        ...
        direction_emoji = "📈" if side == "Buy" else "📉"
        direction_text = "LONG" if side == "Buy" else "SHORT"
        approach_emoji = "🛡️" if selected_approach == "conservative" else "🛡️"
        approach_text = "Conservative Limits" if selected_approach == "conservative" else "Conservative Limits"
        ...
        if limit_num == 1:
            ...
            limit_msg = (
                f"📊 <b>Edit Limit Orders - Streamlined Flow</b>\n"
                f"Symbol: {context.chat_data.get(SYMBOL, 'Unknown')} | Side: {context.chat_data.get(SIDE, 'Buy')}\n\n"
                f"✅ Limit 1: <code>{format_price(context.chat_data.get(LIMIT_ENTRY_1_PRICE))}</code>\n\n"
                f"🎯 <b>Limit Order 2 of 3</b>\n\n"
            )
            ...
            limit_msg += (
                ...
                f"💡 This will be your second entry point"
            )

            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("⬅️ Back to Limit 1", callback_data="ggshot_limit_back:1")],
...
# =============================================
# PRIMARY ENTRY HANDLER (Conservative Approach Only)
# =============================================
...
async def limit_entries_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    logger.info("🎯 limit_entries_handler called - this handler should process limit prices")
...
async def handle_margin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data.startswith("conv_margin_pct:"):
        ...
        if margin_value == "custom":
        ...
        elif margin_value == "custom_percentage":
            # Show custom percentage input (aligned with mobile layout)
            custom_margin_msg = (
                f"📊 <b>Custom Margin Percentage</b>\n\n"
                f"Enter your custom percentage (without % sign):\n"
                f"💡 Example: 3.5 (for 3.5% of account)\n"
            ...
            context.chat_data["expecting_percentage_input"] = True
            return MARGIN
        elif margin_value == "custom_amount":
            # Show custom USDT amount input (explicit USDT mode)
            custom_margin_msg = (
                f"💵 <b>Custom Margin Amount (USDT)</b>\n\n"
                f"Enter your custom margin amount in USDT:\n"
                f"💡 Example: 75\n"
                f"⚠️ This will use a fixed USDT amount, not percentage"
            )
            ...
            try:
                ...
                logger.error(f"Error editing message for custom margin amount: {e}")

            # Set flag to indicate we're expecting USDT input
            context.chat_data["expecting_usdt_input"] = True
            return MARGIN
        else:
...
async def margin_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handle custom margin amount text input - ALWAYS TREATS INPUT AS PERCENTAGE unless expecting_usdt_input flag is set"""
    if not update.message or not update.message.text:
        return MARGIN
    ...
    # Check if we're expecting USDT input (from mobile layout custom amount button)
    expecting_usdt = context.chat_data.get("expecting_usdt_input", False)
    
    # Clear the flags
    context.chat_data.pop("expecting_percentage_input", None)
    context.chat_data.pop("expecting_usdt_input", None)

    try:
        input_text = update.message.text.strip().replace('%', '').replace('$', '')  # Remove % and $ signs if included
        ...
        if expecting_usdt:
            ...
            if input_value <= 0:
                await send_error_and_retry(
                    context, chat_id,
                    "Amount must be greater than 0. Please enter a valid USDT amount:",
                    MARGIN
                )
                return MARGIN

            # Store margin as USDT amount
            margin_usdt = input_value
            
            # Calculate percentage for reference
            from utils.cache import get_usdt_wallet_balance_cached
            try:
                total_balance, available_balance = await get_usdt_wallet_balance_cached()
                margin_percentage = (margin_usdt / available_balance * 100) if available_balance > 0 else Decimal("0")
            except:
                margin_percentage = Decimal("0")
            
            logger.info(f"Custom margin USDT amount entered: {margin_usdt} USDT ({margin_percentage}% of balance)")
        else:
            # Handle percentage input (default behavior)
            if input_value <= 0 or input_value > 100:
                await send_error_and_retry(
                    context, chat_id,
                    "Percentage must be between 0.01 and 100. Please enter a valid percentage:",
                    MARGIN
                )
                return MARGIN
        ...
        context.chat_data[MARGIN_AMOUNT] = margin_usdt
        context.chat_data["MARGIN_AMOUNT"] = margin_usdt
        context.chat_data["margin_percentage"] = margin_percentage  # Store percentage for reference
        ...
        if expecting_usdt:
            await send_error_and_retry(
                context, chat_id,
                "Please enter a valid USDT amount (e.g., 100 for $100):",
                MARGIN
            )
        else:
            await send_error_and_retry(
                context, chat_id,
                "Please enter a valid percentage (e.g., 2 for 2% or 0.5 for 0.5%):",
                MARGIN
            )
        return MARGIN
...
# REMOVED: ask_for_margin_fast function - fast approach no longer supported
async def ask_for_margin_fast_REMOVED(context: ContextTypes.DEFAULT_TYPE, chat_id: int, query: CallbackQuery = None) -> int:
    ...
    margin_msg = (
        f"⚡ <b>FAST APPROACH - Margin Selection</b>\n"
        f"💰 <b>{balance_display}</b>\n\n"
        f"📊 <b>Step 7b: Select Margin for FAST approach</b>\n\n"
        f"Choose what percentage of your account to use for the FAST trade:\n"
        f"💡 This is separate from the Conservative margin\n"
    ...
    # Add custom and cancel buttons
    keyboard.append([
        InlineKeyboardButton(f"✏️ Custom %", callback_data="conv_margin_fast:custom")
    ])
    keyboard.append([
        InlineKeyboardButton("❌ Cancel", callback_data="cancel_conversation")
    ])
...
async def ask_for_margin_conservative(context: ContextTypes.DEFAULT_TYPE, chat_id: int, query: CallbackQuery = None) -> int:
    """Ask for margin for CONSERVATIVE approach"""
    leverage = context.chat_data.get(LEVERAGE, 10)
    ...
    margin_msg = (
        f"🛡️ <b>CONSERVATIVE APPROACH - Margin Selection</b>\n"
        f"💰 <b>{balance_display}</b>\n\n"
        f"📊 <b>Step 7: Select Margin for your trade</b>\n\n"
        f"Choose what percentage of your account to use for this trade:\n"
        f"💡 This determines your position size\n"
    ...
    # Add custom, back and cancel buttons
    keyboard.append([
        InlineKeyboardButton(f"✏️ Custom %", callback_data="conv_margin_cons:custom")
    ])
    keyboard.append([
        InlineKeyboardButton("⬅️ Back", callback_data=f"conv_back:{MARGIN}"),
        InlineKeyboardButton("❌ Cancel", callback_data="cancel_conversation")
    ])
...
async def handle_margin_conservative_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if value == "custom":
        # Show custom percentage input
        custom_margin_msg = (
            f"🛡️ <b>CONSERVATIVE Approach - Custom Margin</b>\n\n"
        ...
        try:
            ...
            if fast_margin + margin_usdt > available_balance:
                await query.answer(
                    f"❌ Total margin (${format_decimal_or_na(fast_margin + margin_usdt, 2)}) exceeds available balance",
                    show_alert=True
                )
                return MARGIN
...
async def show_final_confirmation(context: ContextTypes.DEFAULT_TYPE, chat_id: int) -> int:
    ...
    if display_strategy == "fast" or order_strategy == STRATEGY_MARKET_ONLY:
        ...
        # Calculate WEIGHTED average entry price for conservative trades
        from config.constants import MARKET_ORDER_PERCENTAGE, LIMIT_ORDER_ALLOCATION
        ...
        try:
            ...
            # Fallback to first limit if can't get market price
            market_price = limit1_price if limit1_price > 0 else Decimal("0")
        
        # Calculate weighted average with new allocation
        if all([limit1_price, limit2_price, limit3_price]) and market_price > 0:
            total_weight = Decimal("0")
            weighted_sum = Decimal("0")
            ...
            remaining_weight = Decimal("1") - MARKET_ORDER_PERCENTAGE
            limit_prices = [limit1_price, limit2_price, limit3_price]
            ...
            if total_weight > 0:
                ...
                logger.info(f"📊 Using weighted average entry for R:R: ${avg_entry:.2f}")
            else:
                # Fallback to simple average
                avg_entry = (limit1_price + limit2_price + limit3_price) / 3
        else:
            # Fallback to simple average if not all prices available
...
async def show_both_trades_confirmation(context: ContextTypes.DEFAULT_TYPE, chat_id: int, query: CallbackQuery = None) -> int:
    ...
    confirmation_msg = (
        ...
        f"💰 <b>Total Margin:</b> ${format_decimal_or_na(total_margin, 2)} USDT\n"
    )

    if has_separate_margins:
        confirmation_msg += f"   ⚡ Fast: {fast_percentage}% = ${format_decimal_or_na(fast_margin, 2)}\n"
        confirmation_msg += f"   🛡️ Conservative: {conservative_percentage}% = ${format_decimal_or_na(conservative_margin, 2)}\n"
    else:
    ...
    confirmation_msg += (
        ...
        f"💰 Margin: ${format_decimal_or_na(fast_margin, 2)} USDT"
    )
...
async def handle_execute_trade(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        # Only conservative approach supported now - fast approach logic removed
        ...
        if isinstance(result, dict) and result.get("message"):
            # Use the rich formatted message from trader.py with retry logic
        ...
        # Try to send error message with retry, but don't fail if it doesn't work
        try:
            await send_message_with_retry(
                context.bot,
                chat_id,
                ...
                parse_mode=ParseMode.HTML,
                max_retries=1  # Fewer retries for error messages
            )
...
async def handle_modify_trade(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handle modify trade button - go back to approach selection"""
    query = update.callback_query
    try:
        await query.answer("🔧 Modifying trade setup...")
    except:
        pass

    # Preserve the symbol and side, but allow re-selection of approach
    symbol = context.chat_data.get(SYMBOL, "BTCUSDT")
    side = context.chat_data.get(SIDE, "Buy")
    direction_emoji = "📈" if side == "Buy" else "📉"
    direction_text = "LONG" if side == "Buy" else "SHORT"

    # Clear approach-specific data
    keys_to_clear = [
    ...
    # Show approach selection again
    approach_msg = (
        f"✅ <b>Symbol:</b> <code>{symbol}</code> 🛡️\n"
        f"✅ <b>Direction:</b> {direction_emoji} {direction_text}\n\n"
        f"🔧 <b>Modifying Trade Setup</b>\n\n"
        f"Choose your trading approach:\n\n"
        f"🛡️ <b>Conservative Limits</b>\n"
        f"• Single entry + single TP\n"
        f"• Quick setup\n\n"
        f"🛡️ <b>Conservative Limits</b>\n"
    ...
    try:
        ...
        logger.error(f"Error editing message for modify trade: {e}")

    # Return to approach selection state
    return APPROACH_SELECTION
...
async def execute_both_trades(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if "margin_amount_fast" in context.chat_data and "margin_amount_conservative" in context.chat_data:
        execution_msg = (
            ...
            f"💰 <b>Fast Margin:</b> ${format_decimal_or_na(fast_margin, 2)} ({context.chat_data.get('margin_percentage_fast', 0)}%)\n"
            f"💰 <b>Conservative Margin:</b> ${format_decimal_or_na(conservative_margin, 2)} ({context.chat_data.get('margin_percentage_conservative', 0)}%)\n\n"
        ...
        execution_msg = (
            ...
            f"{get_emoji('loading')} Preparing dual trade execution\n"
            f"📊 This will open 2 separate positions\n"
            f"💰 Margin will be split equally (${format_decimal_or_na(fast_margin, 2)} each)\n\n"
            f"Please wait while we execute both strategies..."
        )
    ...
    try:
        ...
        summary_msg += (
            f"\n📊 <b>Total Margin Used:</b> ${format_decimal_or_na(total_margin_used, 2)} USDT\n"
        )

        # Show breakdown if using separate margins
        if "margin_amount_fast" in context.chat_data and "margin_amount_conservative" in context.chat_data:
            summary_msg += (
                f"   ⚡ Fast: ${format_decimal_or_na(fast_margin, 2)} ({context.chat_data.get('margin_percentage_fast', 0)}%)\n"
                f"   🛡️ Conservative: ${format_decimal_or_na(conservative_margin, 2)} ({context.chat_data.get('margin_percentage_conservative', 0)}%)\n"
            )
...
async def show_ggshot_edit_limits(context: ContextTypes.DEFAULT_TYPE, chat_id: int) -> int:
    ...
    edit_msg = (
        ...
        f"• Entry #1: {limit1_display}\n"
        f"• Entry #2: {limit2_display}\n"
        f"• Entry #3: {limit3_display}\n\n"
    )

    # Add instruction based on edit status
    if edited_fields:
        edit_msg += "💡 <i>Continue editing or click Done when finished</i>\n\n"
    else:
    ...
    btn1_text = "📝 Edit Entry #1" if "limit_1" not in edited_fields else "✏️ Edit Entry #1 (Modified)"
    btn2_text = "📝 Edit Entry #2" if "limit_2" not in edited_fields else "✏️ Edit Entry #2 (Modified)"
    btn3_text = "📝 Edit Entry #3" if "limit_3" not in edited_fields else "✏️ Edit Entry #3 (Modified)"
...
async def handle_ggshot_confirm_all(context: ContextTypes.DEFAULT_TYPE, chat_id: int) -> int:
    ...
    # Only conservative strategy supported now
    strategy_type = "conservative"
...
async def ggshot_edit_value_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if editing_field.startswith("limit_"):
            ...
            # For SL, entry price, return to main confirmation - only conservative approach supported
            strategy_type = "conservative"
...
async def start_ggshot_limit_flow(context: ContextTypes.DEFAULT_TYPE, chat_id: int) -> int:
    ...
    limit_msg += (
        ...
        f"💡 This will be your first entry point"
    )
...
async def handle_ggshot_limit_1_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        limit_msg = (
            f"📊 <b>Edit Limit Orders - Streamlined Flow</b>\n"
            f"Symbol: {context.chat_data.get(SYMBOL, 'Unknown')} | Side: {context.chat_data.get(SIDE, 'Buy')}\n\n"
            f"✅ Limit 1: <code>{format_price(new_price)}</code>\n\n"
            f"🎯 <b>Limit Order 2 of 3</b>\n\n"
        )
...
async def handle_ggshot_limit_2_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        limit_msg = (
            ...
            f"Symbol: {context.chat_data.get(SYMBOL, 'Unknown')} | Side: {context.chat_data.get(SIDE, 'Buy')}\n\n"
            f"✅ Limit 1: <code>{format_price(context.chat_data.get(LIMIT_ENTRY_1_PRICE))}</code>\n"
            f"✅ Limit 2: <code>{format_price(new_price)}</code>\n\n"
            f"🎯 <b>Limit Order 3 of 3</b>\n\n"
        )
        ...
        limit_msg += (
            ...
            f"💡 This will be your third entry point"
        )

        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("⬅️ Back to Limit 2", callback_data="ggshot_limit_back:2")],
            [InlineKeyboardButton("❌ Cancel Edit", callback_data="ggshot_back_to_edit")]
        ])
...
async def handle_ggshot_limit_3_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        limit_msg = (
            f"✅ <b>Limit Orders Updated Successfully!</b>\n\n"
            f"Symbol: {context.chat_data.get(SYMBOL, 'Unknown')} | Side: {context.chat_data.get(SIDE, 'Buy')}\n\n"
            f"📊 <b>Your Limit Orders:</b>\n"
            f"• Limit 1: <code>{format_price(context.chat_data.get(LIMIT_ENTRY_1_PRICE))}</code>\n"
            f"• Limit 2: <code>{format_price(context.chat_data.get(LIMIT_ENTRY_2_PRICE))}</code>\n"
            f"• Limit 3: <code>{format_price(new_price)}</code>\n\n"
        ...
        keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("✅ Confirm All Changes", callback_data="ggshot_back_to_edit")]
        ])
...
async def handle_ggshot_tp_3_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        tp_msg += (
            ...
            f"💡 This is your final profit target"
        )

        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("⬅️ Back to TP3", callback_data="ggshot_tp_back:3")],
            [InlineKeyboardButton("❌ Cancel Edit", callback_data="ggshot_back_to_edit")]
        ])
