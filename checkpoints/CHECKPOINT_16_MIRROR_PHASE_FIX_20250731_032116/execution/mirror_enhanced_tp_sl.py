    ...
    async def setup_mirror_tp_sl_orders(self, symbol: str, side: str, position_size: str,
        ...
        try:
            ...
            monitor_data = {
                ...
                "approach": "conservative" if len(tp_prices) > 1 else "conservative",
            ...
            if result and chat_id and self.main_manager:
                try:
                    ...
                    logger.info(f"✅ Created dashboard monitor for mirror {symbol} {side}")
                except Exception as e:
...
class MirrorEnhancedTPSL:
    ...
    async def _calculate_target_size_with_limits(self, symbol: str, side: str, current_size: Decimal) -> Decimal:
        """
        Calculate target position size including unfilled limit orders
        This ensures SL covers full intended position like main account
        """
        try:
            ...
            # Calculate unfilled limit order quantity
            ...
            for order in orders:
                ...
                if (order.get('orderType') == 'Limit' and 
                    ...
                    logger.info(f"Found unfilled limit order: {order.get('orderId')[:8]}... qty={order.get('qty')}")
            
            # Target size is current position + unfilled limits
            target_size = current_size + unfilled_limit_qty
            
            if unfilled_limit_qty > 0:
                logger.info(f"📊 Target size calculation for {symbol}:")
            ...
            logger.error(f"Error calculating target size with limits: {e}")
            # Fallback to current size if calculation fails
            return current_size
    ...
    async def setup_tp_sl_from_monitor(self, monitor_data: Dict, mirror_position: Dict,
        ...
        try:
            ...
            # Calculate target size including pending limit orders
            target_size = await self._calculate_target_size_with_limits(symbol, side, position_size)
            ...
            for tp_price, tp_qty, tp_level in consolidated_tps:
                try:
                    ...
                    if order_result['retCode'] == 0:
                        ...
                        logger.info(f"✅ Mirror TP{i+1} placed: {tp_qty} @ {tp_price}")
                    else:
            ...
            # Place SL order with full coverage including limits
            try:
                sl_side = tp_side  # Same as TP side

                # Use target size for SL to cover full position including unfilled limits
                sl_quantity = self.validate_quantity(symbol, target_size)
                logger.info(f"📊 SL will cover full target position: {sl_quantity} (current: {position_size}, unfilled: {target_size - position_size})")
                ...
                if sl_result['retCode'] == 0:
                    ...
                    logger.info(f"✅ Mirror SL placed: {sl_quantity} @ {sl_price} (covers full target: {target_size})")
                else:
            ...
            # Update monitor data with order info and target size
            ...
            # Update monitor with target size information
            if monitor_data:
                monitor_data["target_size"] = str(target_size)
                monitor_data["position_size"] = str(target_size)  # Full intended size
                monitor_data["remaining_size"] = str(position_size)  # Current filled size
                logger.info(f"📊 Updated monitor with target size: {target_size}")
...
async def start_mirror_monitoring_task(symbol: str, side: str, enhanced_result: dict):
    ...
    try:
        ...
        logger.info(f"📊 Mirror monitoring task called for {symbol} {side} (handled by manager)")
        return True
    except Exception as e:
