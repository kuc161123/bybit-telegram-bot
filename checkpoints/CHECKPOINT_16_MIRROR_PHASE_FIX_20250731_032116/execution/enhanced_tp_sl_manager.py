        ...
        try:
            ...
            async with self.mirror_sync_locks[mirror_monitor_key]:
                ...
                if mirror_monitor_data:
                    ...
                    logger.info(f"🪞 FINAL STATE: Phase={mirror_monitor_data.get('phase')}, TP1_hit={mirror_monitor_data.get('tp1_hit')}, SL_moved={mirror_monitor_data.get('sl_moved_to_be')}")
    ...
    async def _debug_mirror_phase_status(self, symbol: str, side: str) -> Dict:
        ...
        try:
            ...
            debug_info = {
                ...
                "main_tp1_hit": main_monitor.get("tp1_hit", False) if main_monitor else False,
                "mirror_tp1_hit": mirror_monitor.get("tp1_hit", False) if mirror_monitor else False,
            ...
            logger.info(f"   Main Monitor: Exists={debug_info['main_exists']}, Phase={debug_info['main_phase']}, TP1={debug_info['main_tp1_hit']}")
            logger.info(f"   Mirror Monitor: Exists={debug_info['mirror_exists']}, Phase={debug_info['mirror_phase']}, TP1={debug_info['mirror_tp1_hit']}")
    ...
    async def _transition_to_profit_taking(self, monitor_data: Dict):
        ...
        async with self.phase_transition_locks[monitor_key]:
            ...
            if current_phase in valid_source_phases and current_phase != "PROFIT_TAKING":
                ...
                if CANCEL_LIMITS_ON_TP1 and current_phase == "BUILDING":
                ...
                if account_type == "mirror":
                    logger.info(f"🪞 MIRROR VERIFICATION: Phase={monitor_data.get('phase')}, TP1_hit={monitor_data.get('tp1_hit')}")
                    
            elif current_phase == "PROFIT_TAKING":
                logger.debug(f"🔄 Phase transition skipped for {symbol} {side} ({account_type}) - already in PROFIT_TAKING phase")
                # Ensure tp1_hit flag is set even if already in PROFIT_TAKING
                if not monitor_data.get("tp1_hit", False):
                    monitor_data["tp1_hit"] = True
                    logger.info(f"✅ Updated tp1_hit flag for {symbol} {side} ({account_type})")
            else:
