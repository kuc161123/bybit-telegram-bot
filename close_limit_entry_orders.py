#!/usr/bin/env python3
"""
Close only limit entry orders on both main and mirror accounts
    ...
    for order in orders:
        ...
        try:
            ...
            if response and response.get("retCode") == 0:
                ...
                fail_count += 1
                print(f"❌ {account_name}: Failed to cancel {symbol} order: {response}")
...
async def main():
    """Main execution"""
    print("="*80)
    print("CLOSING LIMIT ENTRY ORDERS ONLY")
    ...
    if main_limit_entries:
        ...
        for order in main_limit_entries:
            ...
            price = order.get("price", "")
            qty = order.get("qty", "")
            print(f"  - {symbol} {side} @ ${price} (qty: {qty})")
