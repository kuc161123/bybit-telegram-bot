...
def cache_dashboard_data(ttl: int = CACHE_TTL):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if cache_key in _dashboard_cache:
                if current_time - _cache_expiry.get(cache_key, 0) < ttl:
...
def clear_dashboard_cache():
    ...
    global _dashboard_cache, _cache_expiry
...
def get_cache_stats() -> Dict[str, Any]:
    ...
    return {
        "cached_items": len(_dashboard_cache),
        "cache_size_bytes": sum(len(str(v)) for v in _dashboard_cache.values()),
        "active_debounce_timers": len(_debounce_timers),
        "oldest_cache_age": min((time.time() - t for t in _cache_expiry.values()), default=0)
    }
