...
async def safe_shutdown():
    """Perform safe shutdown of the bot"""
    ...
    try:
        # 1. Read current pickle state
        print("\n1. Reading current bot state...")
        ...
        if os.path.exists(dashboard_file):
            try:
                ...
                if isinstance(pickle_state, dict):
                    # Check for monitors in different places
                    for chat_id_str, chat_data in pickle_state.items():
                        if isinstance(chat_data, dict) and 'monitors' in chat_data:
                            for monitor_key, monitor_info in chat_data['monitors'].items():
                    ...
                    # Store pickle summary
                    results["pickle_state"] = {
                        "total_chats": len([k for k in pickle_state.keys() if isinstance(k, str) and k.isdigit()]),
                        "has_user_data": bool(pickle_state.get("user_data")),
                        "has_bot_data": bool(pickle_state.get("bot_data")),
                        "file_size": os.path.getsize(dashboard_file)
                    }
            except Exception as e:
                print(f"   Warning: Could not read pickle state: {e}")
        
        # 2. Get current positions and orders from Bybit
        ...
        state_files = [
            ...
            "data/smart_alerts_summary.json"
        ]
        ...
        print("\nTo stop the bot process, follow these steps:")
        print("\n1. First, check if the bot is running:")
        print("   ps aux | grep 'python main.py'")
        print("\n2. Stop the bot using one of these methods:")
        print("   a) Press Ctrl+C in the terminal running the bot")
        print("   b) Run: ./kill_bot.sh")
        print("   c) Run: pkill -f 'python main.py'")
        print("\n3. Verify the bot has stopped:")
        print("   ps aux | grep 'python main.py'")
        print("   (Should show no results)")
