#!/usr/bin/env python3
"""
Fix SL Coverage to Include Unfilled Limit Orders

Updates all stop loss orders to cover 100% including unfilled limit orders
This applies to ALL positions regardless of approach (FAST or CONSERVATIVE)
"""
...
async def fix_sl_coverage_with_limits():
    """Fix SL coverage to include unfilled limit orders for all positions"""
    try:
        ...
        logger.info("🔧 Starting SL Coverage Fix to Include Unfilled Limit Orders")
        logger.info("=" * 80)
        logger.info("📌 This will update ALL positions to have 100% SL coverage")
        ...
        await process_account_sl_coverage(
        ...
        logger.info("📌 All positions now have 100% coverage including unfilled limits")
...
async def process_account_sl_coverage(
    positions: List[Dict],
    orders: List[Dict],
    monitors: Dict,
    account_type: str,
    ...
    """Process SL coverage for an account including unfilled limits"""
    ...
    for position in positions:
        try:
            ...
            # Calculate total intended position (current + unfilled limits)
            intended_position = position_size + total_limit_qty
            
            logger.info(f"   Unfilled limit orders: {len(limit_orders)}")
            if limit_orders:
                for lo in limit_orders:
                    logger.info(f"      - {lo['qty']} @ {lo['price']}")
                logger.info(f"   Total unfilled quantity: {total_limit_qty}")
            
            logger.info(f"   Total intended position: {intended_position}")
            ...
            # Required SL quantity is the total intended position
            required_sl_qty = intended_position
            ...
            logger.info(f"   🔧 Adjusting SL: {current_sl_qty} → {required_sl_qty}")
            ...
            # For mirror account, use mirror cancel function
            ...
            new_link_id = generate_adjusted_order_link_id(old_link_id, "FULL_LIMITS")
            ...
            if sl_result and sl_result.get('orderId'):
                ...
                logger.info(f"      Quantity: {adjusted_qty} (includes {total_limit_qty} unfilled)")
    ...
    logger.info(f"\n📊 {account_type.upper()} Summary: {fixed_count}/{checked_count} positions updated")

if __name__ == "__main__":
    asyncio.run(fix_sl_coverage_with_limits())