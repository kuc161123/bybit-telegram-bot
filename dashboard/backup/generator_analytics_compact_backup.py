...
from clients.bybit_helpers import get_all_positions, get_all_open_orders, get_positions_and_orders_batch
...
async def build_analytics_dashboard_text(chat_data: Any, bot_data: Any) -> str:
    ...
    try:
        ...
        # Get positions and orders in batch for better performance
        positions, all_orders, _, _ = await get_positions_and_orders_batch()
        ...
        # Get all open orders to check for BOT_ prefix
        # all_orders already fetched in batch above
        ...
        for pos in active_positions:
            ...
            # Check if this position has bot orders
            ...
            if has_bot_orders:
                ...
                logger.debug(f"Position {symbol}: Added as bot position (has BOT_ orders)")
            else:
                external_positions.append(pos)
                logger.debug(f"Position {symbol}: Added as external position (no BOT_ orders)")
        ...
        # Use the orders we already fetched above (no need to fetch again)
        # This ensures consistency and avoids potential race conditions
        ...
        for pos in all_positions_for_pnl:
            ...
            if tp_orders:
                ...
                # Debug log all TP orders before sorting
                logger.info(f"DEBUG {symbol}: Found {len(tp_orders)} TP orders before sorting:")
                for i, order in enumerate(tp_orders):
                    price = get_order_price(order)
                    qty = order.get('qty', 0)
                    logger.info(f"  TP{i+1}: price={price}, qty={qty}")

                # Sort TPs by distance from entry price (closest first)
                # For Buy (long): TP prices are above entry, sort ascending (lowest first)
                # For Sell (short): TP prices are below entry, sort descending (highest first)
                tp_orders.sort(key=get_order_price, reverse=(side == 'Sell'))

                # Debug log after sorting
                logger.info(f"DEBUG {symbol}: After sorting (reverse={side == 'Sell'}):")
                for i, order in enumerate(tp_orders):
                    price = get_order_price(order)
                    qty = order.get('qty', 0)
                    logger.info(f"  TP{i+1}: price={price}, qty={qty}")
        ...
        if potential_profit_tp1 > 0 and potential_profit_tp1 > potential_profit_tp1_full:
            logger.warning(f"⚠️ TP1 order profit ({potential_profit_tp1:.2f}) exceeds full position profit ({potential_profit_tp1_full:.2f})!")

        if potential_profit_all_tp > 0 and potential_profit_all_tp < potential_profit_tp1:
            logger.warning(f"⚠️ All TPs profit ({potential_profit_all_tp:.2f}) is less than TP1 profit ({potential_profit_tp1:.2f})!")
        ...
        logger.info(f"  TP1 Orders P&L: +${potential_profit_tp1:.2f} (actual TP1 orders)")
        logger.info(f"  Full Positions @ TP1: +${potential_profit_tp1_full:.2f} (if all positions exit at TP1)")
        logger.info(f"  If All TPs Hit: +${potential_profit_all_tp:.2f} (sum of all TP orders)")
        logger.info(f"  If All SL Hit: -${potential_loss_sl:.2f}")
        logger.info(f"  Risk:Reward = 1:{(potential_profit_tp1_full/potential_loss_sl if potential_loss_sl > 0 else 0):.1f}")
        ...
        dashboard = f"""
📈 <b>═══ ADVANCED ANALYTICS SUITE ═══</b> {now.strftime('%H:%M:%S')}
...
"""
        ...
        # P&L Analysis Explanation:
        # - TP1 Orders: Profit from ACTUAL TP1 orders placed (may be partial positions)
        # - Full Positions @ TP1: Profit if ALL positions exit at TP1 price level
        # - If All TPs Hit: Sum of profits from all TP orders at their respective prices
        # - If All SL Hit: Total loss if all positions hit their stop losses
        # - Risk:Reward: Ratio of potential full TP1 profit to potential SL loss
        dashboard += f"""
...
├ 📊 External Positions: {len(external_positions)}
...
└ 🎯 AI Insight: {ai_analysis.get('ai_insights', 'Analysis based on market conditions')}
"""
