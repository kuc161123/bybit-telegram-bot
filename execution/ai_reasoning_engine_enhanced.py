...
TIMEFRAME: [Expected timeframe for the recommendation to play out]"""
    ...
    def extract_detailed_trade_setup(
        self,
        symbol: str,
        market_data: Dict,
        technical_signals: Dict,
        ...
        """Extract detailed trade setup with live data for enhanced display"""
        try:
            setup = {}
            
            # Calculate entry zone based on live support/resistance
            support = technical_signals.get('support_resistance', {}).get('support', current_price * 0.98)
            resistance = technical_signals.get('support_resistance', {}).get('resistance', current_price * 1.02)
            
            if recommendation == "BUY":
                # Entry zone near support for longs
                entry_lower = support * 0.995  # Just below support
                entry_upper = current_price * 1.005  # Slightly above current
                setup['ai_entry_zone'] = (entry_lower, min(entry_upper, resistance * 0.98))
                
                # Calculate targets based on resistance and ATR
                atr = technical_signals.get('atr', {}).get('value', current_price * 0.02)
                target1 = current_price + (atr * 1.5)
                target2 = current_price + (atr * 2.5)
                target3 = resistance * 1.02  # Above resistance
                setup['ai_targets'] = sorted([target1, target2, target3])
                
                # Stop loss below support
                setup['ai_stop_loss'] = support * 0.98
                setup['ai_invalidation'] = support * 0.99
                
            elif recommendation == "SELL":
                # Entry zone near resistance for shorts
                entry_lower = current_price * 0.995
                entry_upper = resistance * 1.005
                setup['ai_entry_zone'] = (max(entry_lower, support * 1.02), entry_upper)
                
                # Calculate targets based on support and ATR
                atr = technical_signals.get('atr', {}).get('value', current_price * 0.02)
                target1 = current_price - (atr * 1.5)
                target2 = current_price - (atr * 2.5)
                target3 = support * 0.98  # Below support
                setup['ai_targets'] = sorted([target1, target2, target3], reverse=True)
                
                # Stop loss above resistance
                setup['ai_stop_loss'] = resistance * 1.02
                setup['ai_invalidation'] = resistance * 1.01
                
            else:  # HOLD
                # Provide range boundaries
                setup['ai_entry_zone'] = (support * 1.01, resistance * 0.99)
                setup['ai_targets'] = []
                setup['ai_stop_loss'] = None
                setup['ai_invalidation'] = None
            
            # Calculate risk/reward if we have targets and stop
            if setup.get('ai_targets') and setup.get('ai_stop_loss'):
                risk = abs(current_price - setup['ai_stop_loss'])
                reward = abs(setup['ai_targets'][0] - current_price)
                setup['ai_risk_reward'] = round(reward / risk, 1) if risk > 0 else 0
            
            # Extract key signals from patterns and technical indicators
            key_signals = []
            
            # Add dominant patterns
            if pattern_analysis.chart_patterns:
                for p in pattern_analysis.chart_patterns[:2]:  # Top 2 chart patterns
                    key_signals.append(f"{p.pattern_name} ({p.signal})")
            
            if pattern_analysis.candlestick_patterns:
                for p in pattern_analysis.candlestick_patterns[:2]:  # Top 2 candlestick patterns
                    key_signals.append(f"{p.pattern_name} on {p.timeframe}")
            ...
            if rsi.get('value'):
                ...
                elif 'divergence' in str(rsi.get('condition', '')).lower():
                    key_signals.append("RSI Divergence")
            
            macd = technical_signals.get('macd', {})
            if macd.get('crossover'):
                key_signals.append(f"MACD {macd['crossover']} crossover")
            
            # Add volume signal
            volume_ratio = market_data.get('volume_ratio', 1.0)
            if volume_ratio > 1.5:
                key_signals.append(f"High Volume ({volume_ratio:.1f}x avg)")
            elif volume_ratio < 0.5:
                key_signals.append(f"Low Volume ({volume_ratio:.1f}x avg)")
            
            # Add moving average signals
            ma_signals = technical_signals.get('moving_averages', {})
            if ma_signals.get('trend') == 'bullish':
                key_signals.append("Above key MAs")
            elif ma_signals.get('trend') == 'bearish':
                key_signals.append("Below key MAs")
            
            setup['ai_key_signals'] = key_signals[:5]  # Limit to 5 signals
            
            # Pattern confluence
            setup['ai_pattern_confluence'] = pattern_analysis.pattern_confluence
            
            # Count bullish vs bearish patterns
            bullish_count = sum(1 for p in pattern_analysis.chart_patterns + pattern_analysis.candlestick_patterns 
                              if p.signal in ['bullish', 'strong_bullish'])
            bearish_count = sum(1 for p in pattern_analysis.chart_patterns + pattern_analysis.candlestick_patterns 
                              if p.signal in ['bearish', 'strong_bearish'])
            setup['ai_bullish_count'] = bullish_count
            setup['ai_bearish_count'] = bearish_count
            
            # Time horizon based on patterns and volatility
            volatility = market_data.get('volatility_percentage', 2.0)
            if volatility > 5:
                setup['ai_time_horizon'] = "1-2 days"
            elif volatility > 3:
                setup['ai_time_horizon'] = "2-4 days"
            else:
                setup['ai_time_horizon'] = "3-7 days"
            
            # Position sizing based on risk
            if risk_assessment == "LOW":
                setup['ai_position_size'] = "Normal (1-2% risk)"
            elif risk_assessment == "MEDIUM":
                setup['ai_position_size'] = "Reduced (0.5-1% risk)"
            else:  # HIGH
                setup['ai_position_size'] = "Minimal (0.25-0.5% risk)"
            
            return setup
            
        except Exception as e:
            logger.error(f"Error extracting trade setup: {e}")
            return {}
